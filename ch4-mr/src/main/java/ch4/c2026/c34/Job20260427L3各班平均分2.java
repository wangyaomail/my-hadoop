...
public class Job20260427L3各班平均分2 {
    static class MyMapper extends Mapper<LongWritable, Text, Text, Text> {
        ...
        protected void map(LongWritable key, Text value, Mapper<LongWritable, Text, Text, Text>.Context context) throws IOException, InterruptedException {
            ...
            if(toks.length==8){
                if(!sum.containsKey(toks[1])){
                    sum.put(toks[1],Integer.parseInt(toks[7]));
                    counts.put(toks[1],1);
                }else{
                    sum.put(toks[1],sum.get(toks[1])+Integer.parseInt(toks[7]));
                    counts.put(toks[1],1+counts.get(toks[1]));
                }
            }
        }
        ...
        protected void cleanup(Mapper<LongWritable, Text, Text, Text>.Context context) throws IOException, InterruptedException {
            for(String key : sum.keySet()){
                context.write(new Text(key), new Text(counts.get(key)+"\t"+sum.get(key)));
            }
        }
    }
