...
public class Job2025l7 {
    static class MyMapper extends Mapper<LongWritable, Text, Text, Text> {
        ...
        protected void map(LongWritable key, Text value, Mapper<LongWritable, Text, Text, Text>.Context context) throws IOException, InterruptedException {
            ...
            if(toks.length==8){
                String[] names = toks[5].split("");
                for(int i=3;i<names.length-2;i++){
                    if(names[i].equals(names[i+1]) && names[i].equals(names[i+2])){
                        smap.put(toks[1],  smap.getOrDefault(toks[1], 0)+1);
                    }
                }
            }
        }
...
//            context.write(new Text("第1大"), new Text(sortlist.get(0).getKey()+":"+sortlist.get(0).getValue()));
    ...
    public static void main(String[] args) throws IOException, InterruptedException, ClassNotFoundException {
        ...
        Job job = Job.getInstance(conf, Job2025l7.class.getSimpleName());
        job.setJarByClass(Job2025l7.class);
        ...
        FileOutputFormat.setOutputPath(job, new Path("C:\\nos\\my-hadoop\\output\\"+ Job2025l7.class.getSimpleName()));
