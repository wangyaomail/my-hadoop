...
public class Job2025C56PR1 {
    ...
    static class MyReducer2 extends Reducer<IntWritable, Text, Text, Text> {
        ...
        protected void reduce(IntWritable key, Iterable<Text> values, Reducer<IntWritable, Text, Text, Text>.Context context) throws IOException, InterruptedException {
            for (Text val : values) {
                context.write(new Text(val.toString()), new Text((key.get()*-1)+""));
            }
        }
    }
    public static void main(String[] args) throws IOException, InterruptedException, ClassNotFoundException {
        ...
        Job job = Job.getInstance(conf, Job2025C56PR1.class.getSimpleName()+".1");
        job.setJarByClass(Job2025C56PR1.class);
        ...
        String output1 = "C:\\nos\\my-hadoop\\output\\"+ Job2025C56PR1.class.getSimpleName()+".1";
        String output2 = "C:\\nos\\my-hadoop\\output\\"+ Job2025C56PR1.class.getSimpleName()+".2";

        FileOutputFormat.setOutputPath(job, new Path(output1));
        if(job.waitForCompletion(true)){
            job = Job.getInstance(conf, Job2025C56PR1.class.getSimpleName()+".2");
            job.setJarByClass(Job2025C56PR1.class);
