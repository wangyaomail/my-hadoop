...
public class HBase5MRGenderDiff {
    ...
    static class MyMapper extends TableMapper<Text, Text> {
        ...
        protected void map(ImmutableBytesWritable key, Result columns, Context context) throws IOException, InterruptedException {
            Cell cell = columns.getColumnLatestCell(_family, _gender);
            if (cell != null) {
                String rowKey = Bytes.toString(CellUtil.cloneRow(cell));
                String gender = Bytes.toString(CellUtil.cloneValue(cell));
                if(gender.equals("男")){
                    count++;
                } else {
                    count--;
        ...
        protected void cleanup(Mapper<ImmutableBytesWritable, Result, Text, Text>.Context context) throws IOException, InterruptedException {
            context.write(new Text("1"), new Text(""+count));
        }
    }
