...
public class Exp112HotAnswer {
    public static void main(String[] args) throws Exception {
        ...
        job.setJarByClass(Exp112HotAnswer.class);
    ...
    private static class MyMapper extends TableMapper<Text, Text> {
        public FixedTreeMap<Integer, String> mostStarMap = new FixedTreeMap<>(
        ...
        protected void map(ImmutableBytesWritable key, Result value, Mapper<ImmutableBytesWritable, Result, Text, Text>.Context context) throws IOException, InterruptedException {
            Integer star = Integer.parseInt(Bytes.toString(CellUtil.cloneValue(value.getColumnLatestCell(Bytes.toBytes("data"), Bytes.toBytes("star")))));
            mostStarMap.put(star, Bytes.toString(key.get()));
        }
    ...
    private static class MyReducer extends TableReducer<Text, Text, ImmutableBytesWritable> {
        ...
        protected void reduce(Text key, Iterable<Text> values, Reducer<Text, Text, ImmutableBytesWritable, Mutation>.Context context) throws IOException, InterruptedException {
            FixedTreeMap<Integer, String> mostStarMap = new FixedTreeMap<>(
                    new Comparator<Integer>() {
                        @Override
                        public int compare(Integer o1, Integer o2) {
                            return o2 - o1;
            ...
            for (Text val : values) {
                ...
                mostStarMap.put(Integer.parseInt(keyvals[0]), keyvals[1]);
            }
            StringBuilder sb = new StringBuilder();
            mostStarMap.forEach((k, v) -> {
                sb.append(v).append("_").append(k).append(",");
            });
            Put put = new Put(Bytes.toBytes("hotanswer"));
            put.addColumn(Bytes.toBytes("data"), Bytes.toBytes("val"), Bytes.toBytes(sb.toString()));
