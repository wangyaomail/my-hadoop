...
public class Job4PosNegAns {
    ...
    static byte[] _q_mo = Bytes.toBytes("mo");
    ...
    public static void main(String[] args) throws Exception {
        ...
        job.setJarByClass(Job4PosNegAns.class);
    ...
    static class MyMapper extends TableMapper<Text, Text> {
        ...
        protected void map(ImmutableBytesWritable key, Result columns, Context context) throws IOException, InterruptedException {
            ...
            Cell scoreCell = columns.getColumnLatestCell(_family, _q_token);
            if (scoreCell != null) {
                String tokens = Bytes.toString(CellUtil.cloneValue(scoreCell));
                ...
                context.write(new Text(Bytes.toString(CellUtil.cloneRow(scoreCell))),
    ...
    private static class MyReducer extends TableReducer<Text, Text, ImmutableBytesWritable> {
        ...
        protected void reduce(Text key, Iterable<Text> values, Reducer<Text, Text, ImmutableBytesWritable, Mutation>.Context context) throws IOException, InterruptedException {
            for(Text value : values) {
                ...
                String[] posNegStr = value.toString().split(",");
                int pos = Integer.parseInt(posNegStr[0]);
                int neg = Integer.parseInt(posNegStr[1]);
                int mo = pos>neg?1:0;
                put.addColumn(_family, _q_pos, Bytes.toBytes(pos+""));
                put.addColumn(_family, _q_neg, Bytes.toBytes(neg+""));
                put.addColumn(_family, _q_mo, Bytes.toBytes(mo+""));
