                ...
                synchronized (taskTrackers) {
                    synchronized (trackerExpiryQueue) {
                        ...
                        while ((trackerExpiryQueue.size() > 0) && ((leastRecent = (TaskTrackerStatus) trackerExpiryQueue.first()) != null) && (now - leastRecent.getLastSeen() > TASKTRACKER_EXPIRY_INTERVAL)) {
                            ...
                            TaskTrackerStatus newProfile = (TaskTrackerStatus) taskTrackers.get(
                    ...
                    synchronized (jobInitQueue) {
                        synchronized (jobsByArrival) {
                            for (Iterator it = jobs.keySet().iterator(); it.hasNext(); ) {
                                ...
                                if (job.getStatus()
                                       .getRunState() != JobStatus.RUNNING && job.getStatus()
                                                                                 .getRunState() != JobStatus.PREP && (job.getFinishTime() + RETIRE_JOB_INTERVAL < System.currentTimeMillis())) {
                                    it.remove();
        ...
        for (int i = 0; i < mapTasksNeeded.length; i++) {
            for (int j = 0; j < mapTasksNeeded[i].length; j++) {
                ...
                if (tip != null && tip.isComplete(mapTasksNeeded[i][j])) {
                    ...
                    v.add(new MapOutputLocation(mapTasksNeeded[i][j],
                                                tracker.getHost(),
                                                tracker.getPort()));
                    break;
                }
