...
class ReduceTask extends Task {
    ...
    public ReduceTask(
            String jobFile, String taskId, String[][] mapTaskIds, int partition) {
    ...
    public void run(JobConf job, final TaskUmbilicalProtocol umbilical) throws IOException {
        ...
        SequenceFile.Writer writer = new SequenceFile.Writer(lfs, file, keyClass, valueClass);
        try {
            ...
            for (int i = 0; i < mapTaskIds.length; i++) {
                File partFile = this.mapOutputFile.getInputFile(mapTaskIds[i], getTaskId());
        ...
        final RecordWriter out = job.getOutputFormat()
                                    .getRecordWriter(FileSystem.get(job), job, name);
        ...
        try {
            ValuesIterator values = new ValuesIterator(in, length, comparator, umbilical);
            while (values.more()) {
                reducer.reduce(values.getKey(), (Iterator) values, collector, reporter);
                values.nextKey();
            }
