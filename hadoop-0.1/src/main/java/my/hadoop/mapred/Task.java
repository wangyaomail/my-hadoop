...
import my.hadoop.util.Progress;
...
abstract class Task implements Writable, Configurable {
     ...
     */
    public abstract void run(JobConf job, TaskUmbilicalProtocol umbilical) throws IOException;
    ...
    private transient long nextProgressTime = System.currentTimeMillis() + PROGRESS_INTERVAL;
    ...
    public Reporter getReporter(
            final TaskUmbilicalProtocol umbilical, final Progress progress) throws IOException {
        return new Reporter() {
    ...
    public void reportProgress(TaskUmbilicalProtocol umbilical, float progress) throws IOException {
        taskProgress.set(progress);
        reportProgress(umbilical);
    }

    public void reportProgress(TaskUmbilicalProtocol umbilical) throws IOException {
