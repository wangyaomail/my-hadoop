...
import my.hadoop.io.IntWritable;
...
public class TaskTracker
    ...
    public static final Logger LOG = LogFormatter.getLogger("org.apache.hadoop.mapred.TaskTracker");
                ...
                try {
                    ...
                    while (running && !staleState) {
                        try {
                            ...
                            LOG.log(Level.INFO,
                                    "Lost connection to JobTracker [" + jobTrackAddr + "].  Retrying...",
                                    ex);
                            try {
                                Thread.sleep(5000);
                            } catch (InterruptedException ie) {
    ...
    class TaskInProgress {
        ...
        void localizeTask(Task t) throws IOException {
            File localJobFile = this.jobConf.getLocalFile(SUBDIR + File.separator + t.getTaskId(),
                                                          "job.xml");
            File localJarFile = this.jobConf.getLocalFile(SUBDIR + File.separator + t.getTaskId(),
                                                          "job.jar");
    ...
    // MapOutputProtocol
    /////////////////////////////////////////////////////////////////
    public MapOutputFile getFile(
            String mapTaskId, String reduceTaskId, IntWritable partition) {
        MapOutputFile mapOutputFile = new MapOutputFile(mapTaskId, reduceTaskId, partition.get());
     ...
     */
    public synchronized void progress(String taskid, float progress, String state)
            throws IOException {
        TaskInProgress tip = (TaskInProgress) tasks.get(taskid);
        if (tip != null) {
    ...
    public static class Child {
        public static void main(String[] args) throws Throwable {
            ...
            TaskUmbilicalProtocol umbilical = (TaskUmbilicalProtocol) RPC.getProxy(
        ...
        private static void startPinging(
                final TaskUmbilicalProtocol umbilical, final String taskid) {
