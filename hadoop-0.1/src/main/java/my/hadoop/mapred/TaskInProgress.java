...
class TaskInProgress {
    ...
    public boolean shouldCloseForClosedJob(String taskid) {
        ...
        if ((ts != null) && (!tasksReportedClosed.contains(taskid)) && (job.getStatus()
                                                                           .getRunState() != JobStatus.RUNNING)) {
    ...
    TaskReport generateSingleReport() {
        ...
        return new TaskReport(getTIPId(),
                              (float) progress,
                              state,
    ...
    boolean hasSpeculativeTask(double averageProgress) {
        ...
        //
        if (isMapTask() && recentTasks.size() <= MAX_TASK_EXECS && conf.getSpeculativeExecution() && (averageProgress - progress >= SPECULATIVE_GAP) && (System.currentTimeMillis() - startTime >= SPECULATIVE_LAG)) {
            return true;
        }
        return false;
    }
    ...
    public Task getTaskToRun(String taskTracker, TaskTrackerStatus tts, double avgProgress) {
        Task t = null;
        if (hasTaskWithCacheHit(taskTracker, tts) || hasTask() || hasSpeculativeTask(avgProgress)) {
