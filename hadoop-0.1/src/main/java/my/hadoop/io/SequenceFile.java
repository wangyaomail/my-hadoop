...
import my.hadoop.fs.ChecksumException;
...
public class SequenceFile {
    public static final Logger LOG = LogFormatter.getLogger("org.apache.hadoop.io.SequenceFile");
    ...
    public static class Writer {
        ...
        private DeflaterOutputStream deflateFilter = new DeflaterOutputStream(buffer, deflater);
        private DataOutputStream deflateOut = new DataOutputStream(new BufferedOutputStream(
                deflateFilter));
         ...
         */
        public Writer(FileSystem fs, String name, Class keyClass, Class valClass, boolean compress)
                throws IOException {
            this.fs = fs;
         ...
         */
        private Writer(FSDataOutputStream out, Class keyClass, Class valClass, boolean compress)
                throws IOException {
         ...
         */
        private void init(FSDataOutputStream out, Class keyClass, Class valClass, boolean compress)
                throws IOException {
            this.out = out;
         ...
         * Append a key/value pair.
         */
        public synchronized void append(byte[] data, int start, int length, int keyLength)
                throws IOException {
            if (keyLength == 0)
    ...
    public static class Reader {
        ...
        public synchronized boolean next(Writable key) throws IOException {
            ...
            if (inBuf.getPosition() != keyLength)
                throw new IOException(key + " read " + inBuf.getPosition() + " bytes, should read " + keyLength);
        ...
        public synchronized boolean next(Writable key, Writable val) throws IOException {
            ...
            if (more) {
                ...
                if (inflateValues) {
                    inflater.reset();
                    inflater.setInput(outBuf.getData(), keyLength, outBuf.getLength() - keyLength);
                    inflateOut.reset();
                ...
                if (inBuf.getPosition() != inBuf.getLength())
                    throw new IOException(val + " read " + (inBuf.getPosition() - keyLength) + " bytes, should read " + (inBuf.getLength() - keyLength));
            }
        ...
        public synchronized int next(DataOutputBuffer buffer) throws IOException {
            ...
            try {
                ...
                if (version[3] > 1 && sync != null && length == SYNC_ESCAPE) {              // process a sync entry
    ...
    public static class Sorter {
         ...
         */
        public Sorter(
                FileSystem fs, WritableComparator comparator, Class valClass, Configuration conf) {
            this.fs = fs;
        ...
        private class SortPass {
            ...
            private int compare(int i, int j) {
                return comparator.compare(rawBuffer,
                                          starts[i],
                                          keyLengths[i],
                                          rawBuffer,
                                          starts[j],
        ...
        private class MergePass {
            ...
            public MergePass(int pass, boolean last) throws IOException {
                ...
                this.queue = new MergeQueue(factor, last ? outFile : outFile + "." + pass, last);
        ...
        private class MergeFiles {
            ...
            public void run() throws IOException {
                ...
                for (int i = 0; i < inFiles.length; i++) {
                    ...
                    MergeStream ms = new MergeStream(new Reader(fs, inFile, memory / (factor + 1)));
                    if (ms.next())
                        queue.put(ms);
                }
        ...
        private class MergeQueue extends PriorityQueue {
            ...
            public MergeQueue(int size, String outName, boolean done) throws IOException {
                initialize(size);
            ...
            protected boolean lessThan(Object a, Object b) {
                ...
                return comparator.compare(msa.buffer.getData(),
                                          0,
                                          msa.keyLength,
                                          msb.buffer.getData(),
                                          0,
