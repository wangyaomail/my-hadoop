...
import my.hadoop.fs.LocalFileSystem;
...
public class MapFile {
    ...
    public static class Writer {
        ...
        public Writer(
                FileSystem fs, String dirName, Class keyClass, Class valClass) throws IOException {
        ...
        public Writer(FileSystem fs, String dirName, WritableComparator comparator, Class valClass)
                throws IOException {
        ...
        public Writer(
            ...
            this.data = new SequenceFile.Writer(fs,
                                                dataFile.getPath(),
                                                keyClass,
                                                valClass,
                                                compress);
            this.index = new SequenceFile.Writer(fs,
                                                 indexFile.getPath(),
                                                 keyClass,
                                                 LongWritable.class);
        }
        ...
        public synchronized boolean seek(WritableComparable key) throws IOException {
            ...
            if (seekIndex != -1                         // seeked before
                    && seekIndex + 1 < count && comparator.compare(key,
                                                                   keys[seekIndex + 1]) < 0 // before next indexed
                    && comparator.compare(keyBuf.getData(),
                                          0,
                                          keyBuf.getLength(),
                                          nextBuf.getData(),
                                          0,
                                          nextKeyLen) >= 0) {                                 // but after last seeked
                // do nothing
            } else {
            ...
            while ((nextKeyLen = data.next(nextBuf.reset())) != -1) {
                int c = comparator.compare(keyBuf.getData(),
                                           0,
                                           keyBuf.getLength(),
                                           nextBuf.getData(),
                                           0,
    ...
    public static long fix(
        ...
        if (!dataReader.getKeyClass().equals(keyClass)) {
            throw new Exception(dr + "Wrong key class in " + dir + ", expected" + keyClass.getName() + ", got " + dataReader.getKeyClass()
                                                                                                                            .getName());
        }
        if (!dataReader.getValueClass().equals(valueClass)) {
            throw new Exception(dr + "Wrong value class in " + dir + ", expected" + valueClass.getName() + ", got " + dataReader.getValueClass()
                                                                                                                                .getName());
        }
        long cnt = 0L;
        ...
        if (!dryrun)
            indexWriter = new SequenceFile.Writer(fs,
                                                  index.toString(),
                                                  keyClass,
                                                  LongWritable.class);
        try {
    ...
    public static void main(String[] args) throws Exception {
        ...
        MapFile.Writer writer = new MapFile.Writer(fs,
                                                   out,
                                                   reader.getKeyClass(),
