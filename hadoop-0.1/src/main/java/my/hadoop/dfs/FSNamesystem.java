...
class FSNamesystem implements FSConstants {
     ...
     */
    public synchronized Object[] startFile(
            UTF8 src, UTF8 holder, UTF8 clientMachine, boolean overwrite) {
        Object results[] = null;
        if (pendingCreates.get(src) == null) {
            ...
            if (fileValid) {
                ...
                DatanodeInfo targets[] = chooseTargets(this.desiredReplication,
                                                       null,
                                                       clientMachine);
                if (targets.length < this.minReplication) {
                    LOG.warning("Target-length is " + targets.length + ", below MIN_REPLICATION (" + this.minReplication + ")");
                    return null;
                }
    ...
    public synchronized int completeFile(UTF8 src, UTF8 holder) {
        if (dir.getFile(src) != null || pendingCreates.get(src) == null) {
            LOG.info("Failed to complete " + src + "  because dir.getFile()==" + dir.getFile(src) + " and " + pendingCreates.get(
                    src));
    ...
    synchronized void heartbeatCheck() {
        synchronized (heartbeats) {
            ...
            while ((heartbeats.size() > 0) && ((nodeInfo = (DatanodeInfo) heartbeats.first()) != null) && (nodeInfo.lastUpdate() < System.currentTimeMillis() - EXPIRE_INTERVAL)) {
    ...
    public synchronized Block[] processReport(Block newReport[], UTF8 name) {
        ...
        if (node == null) {
            throw new IllegalArgumentException(
                    "Unexpected exception.  Received block report from node " + name + ", but there is no info for " + name);
        }
    ...
    public synchronized void blockReceived(Block block, UTF8 name) {
        ...
        if (node == null) {
            throw new IllegalArgumentException(
                    "Unexpected exception.  Got blockReceived message from node " + name + ", but there is no info for " + name);
        }
        //
                ...
                for (Iterator it = neededReplications.iterator(); it.hasNext(); ) {
                    ...
                    if (!dir.isValidBlock(block)) {
                        ...
                        if (containingNodes.contains(srcNode)) {
                            DatanodeInfo targets[] = chooseTargets(Math.min(this.desiredReplication - containingNodes.size(),
     ...
     */
    DatanodeInfo[] chooseTargets(
            int desiredReplicates, TreeSet forbiddenNodes, UTF8 clientMachine) {
    ...
    DatanodeInfo chooseTarget(TreeSet forbidden1, TreeSet forbidden2, UTF8 clientMachine) {
        ...
        if (targetList.size() > 0) {
            ...
            LOG.warning("Zero targets found, forbidden1.size=" + (forbidden1 != null ?
                    forbidden1.size() :
                    0) + " forbidden2.size()=" + (forbidden2 != null ? forbidden2.size() : 0));
            return null;
        }
    }
}
